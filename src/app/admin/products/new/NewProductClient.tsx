...
import { PackagePlus, ChevronLeft, Image as ImageIcon } from "lucide-react"
import toast from "react-hot-toast"

export default function NewProductClient({ categories }: { categories: { id: string; name: string }[] }) {
  ...
  const [pricePlans, setPricePlans] = useState([
    { name: "天卡", durationDays: 1, price: 100 },
    { name: "周卡", durationDays: 7, price: 500 },
    { name: "月卡", durationDays: 30, price: 1500 }
  ])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const toastId = toast.loading("新增商品中...")
    ...
    try {
      ...
      if (res.ok) {
        toast.success("商品新增成功！", { id: toastId })
        router.push("/admin/products")
        router.refresh()
      } else {
        const err = await res.json()
        toast.error(err.error || "新增失敗", { id: toastId })
      }
    } catch (error) {
      console.error(error)
    ...
    <div className="max-w-4xl mx-auto space-y-8 relative z-10">
      <div className="flex justify-between items-center border-b border-white/10 pb-6 mb-6">
        <div className="flex items-center gap-4">
          <Link href="/admin/products" className="p-2 bg-white/5 hover:bg-white/10 rounded-lg text-slate-300 transition-colors border border-white/10">
            <ChevronLeft className="w-5 h-5" />
          </Link>
          <div className="p-3 bg-secondary/20 rounded-xl border border-secondary/30 shadow-[0_0_15px_rgba(var(--secondary-rgb),0.3)]">
            <PackagePlus className="w-8 h-8 text-secondary drop-shadow-[0_0_8px_rgba(var(--secondary-rgb),0.8)]" />
          </div>
          <div>
            <h1 className="text-3xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400 drop-shadow-sm">新增商品</h1>
            <p className="text-slate-400 mt-1">建立全新的輔助並設定價格方案</p>
          </div>
        </div>
      </div>

      <form onSubmit={handleSubmit} className="glass-card p-8 rounded-2xl border border-white/10 space-y-8 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
            ...
            {pricePlans.map((plan, idx) => (
              <div key={idx} className="flex flex-wrap md:flex-nowrap gap-4 items-end bg-white/5 p-5 rounded-xl border border-white/10 hover:border-white/20 transition-all">
                ...
                <div className="w-1/4 min-w-[100px] space-y-1">
                  ...
                  <input
                    type="number" value={plan.durationDays} min={0}
